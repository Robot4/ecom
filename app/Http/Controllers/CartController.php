...
class CartController extends Controller
{
    ...
    public function addToCart(Request $request)
    {
        ...
        $selectedSize = $request->input('selected_size'); // Get the selected size from the request

        if ($already_cart) {
            $already_cart->quantity = $already_cart->quantity + 1;
            $already_cart->amount = $product->price + $already_cart->amount;
            $already_cart->selected_size = $selectedSize; // Set the selected_size
            ...
            $cart->selected_size = $selectedSize; // Set the selected_size
    ...
    public function singleAddToCart(Request $request){
        $request->validate([
            'slug'      =>  'required',
            'quant'      =>  'required',
            'selected_size' => 'required', // Add validation for selected_size
        ...
        $already_cart = Cart::where('user_id', auth()->user()->id)->where('order_id',null)->where('product_id', $product->id)->where('selected_size', $selectedSize)->first();
